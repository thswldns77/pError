...
- 실제 AWS 리소스는 자동 생성하지 않는다. 기본 검증은 로컬 실행과 Terraform 코드 검사까지만 진행한다.
...
AWS Academy 환경에서도 `EC2`, `ALB`, `RDS`를 켜둔 상태로 방치하면 비용 또는 크레딧 소모가 발생할 수 있습니다. 이 저장소는 로컬 실행과 Terraform 코드 제출을 기본 범위로 하며, 실제 배포 명령은 수동으로만 수행합니다.
